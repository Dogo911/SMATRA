...
def main():
    ...
    try:
        ...
        # Konfiguration
        lidar_interface = "enx606d3ccc5c04"
        lidar_ip = "192.168.0.10"
        lidar_mac = "00:1d:9b:1e:0e:02"

        print(f"\n🧹 Lösche evtl. vorhandene ARP/Route zu {lidar_ip} ...")
        run_command(f"ip neigh del {lidar_ip} dev {lidar_interface}", use_sudo=True, password=password)
        run_command(f"ip route del {lidar_ip}", use_sudo=True, password=password)

        print(f"\n📍 Setze statische ARP für {lidar_ip} → {lidar_mac} auf {lidar_interface} ...")
        run_command(f"arp -s {lidar_ip} {lidar_mac} -i {lidar_interface}", use_sudo=True, password=password)

        print(f"📍 Setze Route über {lidar_interface} ...")
        run_command(f"ip route add {lidar_ip} dev {lidar_interface}", use_sudo=True, password=password)

        print(f"\n🚀 Starte ROS2 Lidar-Node für {lidar_ip} ...")
        run_command(
